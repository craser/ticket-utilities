...
const [getCurrentBranch, setCurrentBranch] = (function () {
    let branches = [
        'bugfix/2222',
    ];
    return [
...
test('Should correctly retrieve current ticket from current client (solo)', () => {
    setWorkingDirectory(SOLO_WKD);
    let ticketManager = new TicketManager();
    return ticketManager.getCurrentTicket()
        .then(ticket => {
            expect(ticket.getTicketId()).toBe('SOLO-2222');
        });
});

test('Should correctly retrieve current ticket from current client (bantha)', () => {
    setWorkingDirectory(PUDU_WKD);
    let ticketManager = new TicketManager();
    return ticketManager.getCurrentTicket()
        .then(ticket => {
            expect(ticket.getTicketId()).toBe('BNTA-2222');
        });
});

test('Should return null for unrecognized branches', () => {
    setWorkingDirectory(SOLO_WKD);
    setCurrentBranch('refactor.disaster');
    let ticketManager = new TicketManager();
    return ticketManager.getCurrentTicket()
        .then(ticket => {
